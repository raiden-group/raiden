...
class Init {
    constructor(program) {
        ...
        this.getTemplateList = async function () {
            ...
            const res  = await Catch.download(p, 'config/template.config.js');
            if (res.code === 0) {
        ...
        this.initTemplate = async function (sourceDir, {packageName, version}) {
            ...
            if (fs.existsSync(sourceDir)) {
                ...
                compressing.tgz.uncompress(sourceDir, exactDir ).then(() => {
        ...
        this.loadTemplate = async function () {
            ...
            if (this.templateName) {
                ...
                const res = await Catch.download(p, `template/${packageName}-${version}.tgz`);
